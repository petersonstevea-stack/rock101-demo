...
function countCompleted(
  ...
  const completed =
    items.filter((item) => isItemEarned(item, curriculum[item.id])).length;

  return {
    completed,
    total: items.length,
    percent: items.length ? clampPercent((completed / items.length) * 100) : 0,
  };
}
...
function getCertificateStatus(
  ...
  const allRequiredItems = [...privateLessonItems, ...groupRehearsalItems]
    .filter(
      (item) => item.required,
    );
  ...
  const completedRequired =
    allRequiredItems.filter((item) => isItemEarned(item, curriculum[item.id]))
      .length;
...
function getRehearsalReady(
  ...
  const ready = rehearsalReadinessPercent >= 80 &&
    methodAppLessonsPercent >= 70;
...
export function buildParentDashboardData({
  ...
  badges = [],
  classFeedback = null,
  lessonAuthorName = null,
  rehearsalAuthorName = null,
  ...
  return {
    ...
    stats: {
      privateLessons: {
        ...
        sublabel:
          `${privateLessons.completed}/${privateLessons.total} complete`,
      },
      graduationRequirements: {
        label: "Graduation Requirements",
        value: `${graduationRequirements.percent}%`,
        sublabel:
          `${graduationRequirements.completed}/${graduationRequirements.total} complete`,
      },
      groupRehearsal: {
        label: "Group Rehearsal",
        value: `${groupRehearsal.percent}%`,
        sublabel:
          `${groupRehearsal.completed}/${groupRehearsal.total} complete`,
      },
      badgesEarned: {
        label: "Badges Earned",
        value: `${badgesEarned}`,
